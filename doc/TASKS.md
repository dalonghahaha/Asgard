...
- [ ] **T-001** 选定前端框架（Vue 3 / React 18 / Svelte 5 / Nuxt 3 / Next.js 13+）
- [ ] **T-002** 选定构建工具（Vite / Webpack 5 / Turbopack）
- [ ] **T-003** 选定 UI 组件库（Element Plus / Ant Design Vue / Naive UI / Arco Design / shadcn-vue）
- [ ] **T-004** 选定 HTTP 客户端与拦截策略（axios / ofetch / ky / fetch wrapper）
- [ ] **T-005** 选定状态管理（Pinia / Vuex / Zustand / Redux Toolkit / valtio）
- [ ] **T-006** 决定 TypeScript 还是 JavaScript（推荐 TS）
- [ ] **T-007** 决定前端工程位置（仓库内子目录 `web-admin/` / 独立 repo `asgard-frontend`）
- [ ] **T-008** 决定后端 API 路由前缀（建议 `/api/v1`，版本化便于后续兼容）
- [ ] **T-009** 决定鉴权协议：保留现有 DES 加密 cookie 还是改 JWT Bearer Token（影响 `web/middlewares/login.go` + 前端 storage 选型）
- [ ] **T-010** 决定实时数据方案：是否需要 SSE / WebSocket（用于实时监控图表、实时日志）

### Phase 1：后端 API 化

> 目标：让前端能拿到与现有 HTML 路由等价的数据。**不删任何 HTML 路由**，先做增量。

- [ ] **T-101** 在 `web/server.go` 注册新的 `api` 路由 group（`server.Group("/api/v1")`），与现有 HTML 路由并存
